...
export class Decoder {
  ...
  private isSupplyLendingTransaction() {
    return !!this.getMoveCallTransaction(`${this._protocolId}::mint::mint`);
  }

  private decodeSupplyLending(): DecodeResult {
    const coinName = this.helper.decodeOwnedObjectId(0);
  ...
  private get helper() {
    const moveCall = this.transactions.find(
      (trans) => trans.kind === 'MoveCall' && trans.target.startsWith(this._protocolId),
