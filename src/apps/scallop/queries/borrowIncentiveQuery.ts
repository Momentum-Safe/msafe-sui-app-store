...
export const queryBorrowIncentivePools = async (
  ...
  borrowIncentiveCoinNames = borrowIncentiveCoinNames || [...SUPPORT_BORROW_INCENTIVE_POOLS];
  ...
  const rewardCoinName = query.utils.parseCoinNameFromType<SupportAssetCoins>(rewardCoinType);
  const coinPrices = await query.utils.getCoinPrices([...new Set([...borrowIncentiveCoinNames, rewardCoinName])] ?? []);

  const borrowIncentivePools: BorrowIncentivePools = {};

  if (indexer) {
    const borrowIncentivePoolsIndexer = await query.indexer.getBorrowIncentivePools();
    for (const borrowIncentivePool of Object.values(borrowIncentivePoolsIndexer)) {
  ...
  for (const pool of borrowIncentivePoolsQueryData.incentive_pools) {
    ...
    const coinName = query.utils.parseCoinNameFromType<SupportBorrowIncentiveCoins>(coinType);
    const rewardCoinName = query.utils.parseCoinNameFromType<SupportAssetCoins>(rewardCoinType);
...
export const queryBorrowIncentiveAccounts = async (
  ...
  const borrowIncentiveAccounts: BorrowIncentiveAccounts = Object.values(
    ...
    const coinName = query.utils.parseCoinNameFromType<SupportBorrowIncentiveCoins>(poolType);
