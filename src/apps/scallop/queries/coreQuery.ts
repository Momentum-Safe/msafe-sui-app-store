...
import { SUPPORT_POOLS, PROTOCOL_OBJECT_ID, SUPPORT_COLLATERALS, BORROW_FEE_PROTOCOL_ID } from '../constants';
 ...
 * @return Market data.
 */
export const queryMarket = async (query: ScallopQuery, indexer = false) => {
  ...
  if (indexer) {
    ...
    for (const collateral of Object.values(marketIndexer.collaterals)) {
      ...
      collateral.coinWrappedType = query.utils.getCoinWrappedType(collateral.coinName);
    }
    return {
  ...
  for (const pool of marketData.pools) {
    ...
    const poolCoinName = query.utils.parseCoinNameFromType<SupportPoolCoins>(coinType);
    ...
    const calculatedMarketPoolData = calculateMarketPoolData(query.utils, parsedMarketPoolData);
  ...
  for (const collateral of marketData.collaterals) {
    ...
    const collateralCoinName = query.utils.parseCoinNameFromType<SupportCollateralCoins>(coinType);
    ...
    const calculatedMarketCollateralData = calculateMarketCollateralData(query.utils, parsedMarketCollateralData);
 ...
 */
export const getMarketPools = async (query: ScallopQuery, poolCoinNames?: SupportPoolCoins[], indexer = false) => {
  ...
  if (indexer) {
    ...
    for (const marketPool of Object.values(marketPoolsIndexer)) {
      ...
      marketPool.coinWrappedType = query.utils.getCoinWrappedType(marketPool.coinName);
...
export const getMarketPool = async (
  ...
  if (indexer) {
    ...
    marketPoolIndexer.coinWrappedType = query.utils.getCoinWrappedType(marketPoolIndexer.coinName);
  ...
  if (marketObject) {
    if (marketObject.content && 'fields' in marketObject.content) {
      ...
      if (
        ...
        const dynamicFields = balanceSheetDdynamicFieldObject.content.fields as any;
      ...
      if (
        ...
        const dynamicFields = borrowIndexDynamicFieldObject.content.fields as any;
      ...
      if (
        ...
        const dynamicFields = interestModelDynamicFieldObject.content.fields as any;
...
export const getMarketCollaterals = async (
  ...
  if (indexer) {
    ...
    for (const marketCollateral of Object.values(marketCollateralsIndexer)) {
      ...
      marketCollateral.coinWrappedType = query.utils.getCoinWrappedType(marketCollateral.coinName);
...
export const getMarketCollateral = async (
  ...
  if (indexer) {
    ...
    marketCollateralIndexer.coinWrappedType = query.utils.getCoinWrappedType(marketCollateralIndexer.coinName);
  ...
  if (marketObject) {
    if (marketObject.content && 'fields' in marketObject.content) {
      ...
      if (
        ...
        const dynamicFields = riskModelDdynamicFieldObject.content.fields as any;
      ...
      if (
        ...
        const dynamicFields = collateralStatDynamicFieldObject.content.fields as any;
...
export const getMarketCoinAmounts = async (
  ...
  for (const marketCoinObject of marketCoinObjects) {
    ...
    if (marketCoinObject.content && 'fields' in marketCoinObject.content) {
      ...
      const marketCoinName = query.utils.parseCoinNameFromType<SupportMarketCoins>(marketCoinType);
      if (marketCoinName) {
        marketCoinAmounts[marketCoinName] = BigNumber(marketCoinAmounts[marketCoinName] ?? 0)
          .plus(fields.balance)
          .toNumber();
      }
    }
