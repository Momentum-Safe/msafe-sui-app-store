...
export const parseOriginBorrowIncentivePoolData = (
  ...
  pointPerPeriod: Number(originBorrowIncentivePoolData.distributed_point_per_period),
...
export const calculateBorrowIncentivePoolData = (
  ...
  const distributedPointPerSec = BigNumber(parsedBorrowIncentivePoolData.pointPerPeriod).dividedBy(
    parsedBorrowIncentivePoolData.period,
  );

  const pointPerSec = BigNumber(parsedBorrowIncentivePoolData.pointPerPeriod).dividedBy(
    parsedBorrowIncentivePoolData.period,
  );
  ...
  const timeDelta = BigNumber(Math.floor(new Date().getTime() / 1000) - parsedBorrowIncentivePoolData.lastUpdate)
    .dividedBy(parsedBorrowIncentivePoolData.period)
    .toFixed(0);
  const remainingPoints = BigNumber(parsedBorrowIncentivePoolData.maxPoint).minus(
    parsedBorrowIncentivePoolData.distributedPoint,
  );
  const accumulatedPoints = BigNumber.minimum(
    BigNumber(timeDelta).multipliedBy(parsedBorrowIncentivePoolData.pointPerPeriod),
    remainingPoints,
  );
  ...
  const currentPointIndex = BigNumber(parsedBorrowIncentivePoolData.index).plus(
    accumulatedPoints.dividedBy(parsedBorrowIncentivePoolData.staked).isFinite()
      ? BigNumber(baseIndexRate).multipliedBy(accumulatedPoints).dividedBy(parsedBorrowIncentivePoolData.staked)
      : 0,
  );
  const currentTotalDistributedPoint = BigNumber(parsedBorrowIncentivePoolData.distributedPoint).plus(
    accumulatedPoints,
  );
...
export const parseOriginBorrowIncentiveRewardPoolData = (
  ...
  rewardType: normalizeStructTag(originBorrowIncentiveRewardPoolData.reward_type.name),
  claimedRewards: Number(originBorrowIncentiveRewardPoolData.claimed_rewards),
  exchangeRateNumerator: Number(originBorrowIncentiveRewardPoolData.exchange_rate_numerator),
  exchangeRateDenominator: Number(originBorrowIncentiveRewardPoolData.exchange_rate_denominator),
  remainingRewards: Number(originBorrowIncentiveRewardPoolData.remaining_reward),
});
...
export const calculateBorrowIncentiveRewardPoolData = (
  ...
  const rewardPerSec = BigNumber(calculatedBorrowIncentivePoolData.distributedPointPerSec)
  ...
  const totalRewardCoin = totalRewardAmount.shiftedBy(-1 * rewardCoinDecimal);
  const totalRewardValue = totalRewardCoin.multipliedBy(rewardCoinPrice);
  const remaindRewardAmount = BigNumber(parsedBorrowIncentiveRewardPoolData.remainingRewards);
  const remaindRewardCoin = remaindRewardAmount.shiftedBy(-1 * rewardCoinDecimal);
  const remaindRewardValue = remaindRewardCoin.multipliedBy(rewardCoinPrice);
  const claimedRewardAmount = BigNumber(parsedBorrowIncentiveRewardPoolData.claimedRewards);
  const claimedRewardCoin = claimedRewardAmount.shiftedBy(-1 * rewardCoinDecimal);
  const claimedRewardValue = claimedRewardCoin.multipliedBy(rewardCoinPrice);
  ...
  const rewardValueForYear = BigNumber(rewardPerSec)
    ...
    .multipliedBy(rateYearFactor)
    .multipliedBy(rewardCoinPrice);
  const rewardRate = rewardValueForYear.dividedBy(calculatedBorrowIncentivePoolData.stakedValue).isFinite()
    ? rewardValueForYear.dividedBy(calculatedBorrowIncentivePoolData.stakedValue).toNumber()
    : Infinity;
