...
import { getPsmInTxb } from '../api/psm';

export class PsmInIntention extends BaseIntention<BucketIntentionData> {
  ...
  async build(input: { network: SuiNetworks; suiClient: SuiClient; account: WalletAccount }): Promise<Transaction> {
    ...
    const txb = await getPsmInTxb(txbParams, account, network);
    return txb;
  }
