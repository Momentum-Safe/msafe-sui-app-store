...
export const getClmmSdk = (network: SuiNetworks, account: WalletAccount) => {
  const clmmSdk = CetusClmmSDK.createSDK({ env: 'mainnet', sui_client: new SuiClient({ url: getFullnodeUrl('mainnet') }) })
  clmmSdk.setSenderAddress(account.address)
  return clmmSdk
};

export const getFarmsSdk = (network: SuiNetworks, account: WalletAccount) => {
  const farmsSdk = CetusFarmsSDK.createSDK({ env: 'mainnet', sui_client: new SuiClient({ url: getFullnodeUrl('mainnet') }) })
