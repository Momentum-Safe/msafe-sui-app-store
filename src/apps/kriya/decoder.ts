...
import { TransactionBlockInput, Transaction } from '@mysten/sui.js/transactions';
...
export class Decoder {
  ...
  private isAddLiquiditySingleSidedTransaction() {
    return (
      !!this.getMoveCallTransaction(`${config.packageId}::spot_dex::add_liquidity`) &&
      (!!this.getMoveCallTransaction(`${config.packageId}::spot_dex::${config.functions.swapX}`) ||
        !!this.getMoveCallTransaction(`${config.packageId}::spot_dex::${config.functions.swapY}`))
    );
  }
  ...
  private decodeAddLiquiditySingleSided(): DecodeResult {
    ...
    const tokenYType = this.helperLiquidityDex.typeArg(1);
    const inputCoinType =
      this.helperSwapDex.moveCall.target === `${config.packageId}::spot_dex::${config.functions.swapX}`
        ? tokenXType
        : tokenYType;
  ...
  private get helperSwapDex() {
    const moveCall = this.transactions.find(
      (trans) =>
        trans.kind === 'MoveCall' &&
        (trans.target === `${config.packageId}::spot_dex::${config.functions.swapX}` ||
          trans.target === `${config.packageId}::spot_dex::${config.functions.swapY}`),
    ) as MoveCallTransaction;
    return new MoveCallHelper(moveCall, this.txb);
  }
